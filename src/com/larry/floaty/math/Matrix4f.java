...
import com.larry.floaty.utils.BufferUtils;
...
public class Matrix4f {
	...
	private static Matrix4f identity() {//x * identity = x
	...
	public static Matrix4f orthographic(float left, float right, float bottom, float top, float near, float far){//for the shaders to know wazzup
	...
	public static Matrix4f flip(float angle){
		Matrix4f result = identity();
		float r = (float) Math.toRadians(angle);
		float cos = (float) Math.cos(r);
		float sin = (float) Math.sin(r);
